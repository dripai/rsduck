...
fn pg_session_setting_probes_are_accepted() {
    ...
    let setting_result = compat_result("SELECT current_setting('server_version_num');", "admin")
        .expect("compat result");
    ...
    let set_result = compat_result("SET extra_float_digits = 3;", "admin").expect("compat result");
    match set_result {
        SqlResult::Execute {
            command,
            affected_rows,
...
fn pg_class_rewrite_returns_partitioned_table_relkind() {
    ...
    let sql = rewrite_sql("SELECT relname, relkind FROM pg_catalog.pg_class").expect("rewrite sql");
    assert_eq!(route_sql(&sql).unwrap().route, SqlRoute::Read);
...
fn navicat_partitioned_table_column_query_rewrites_any_conkey() {
    ...
    assert!(rows
        .iter()
        .all(|(_, primarykey, _, autoincrement)| { !primarykey && !autoincrement }));
}
...
fn pg_attribute_rewrite_only_returns_dropped_columns_when_requested() {
    ...
    .unwrap();
    crate::catalog::execute_catalog_aware_write(&conn, "ALTER TABLE kline_day DROP COLUMN close")
        .unwrap();
...
fn pg_class_tables_and_information_schema_tables_have_distinct_shapes() {
    ...
    let tables_sql =
        rewrite_sql("SELECT tablename FROM pg_catalog.pg_tables").expect("rewrite pg_tables sql");
    let pg_tables_index_count: i64 = conn
        .query_row(
            &format!("SELECT COUNT(*) FROM ({tables_sql}) WHERE tablename = 'idx_kline_day_code'"),
            [],
            |row| row.get(0),
        )
        .unwrap();
    ...
    let info_index_count: i64 = conn
        .query_row(
            &format!("SELECT COUNT(*) FROM ({info_sql}) WHERE table_name = 'idx_kline_day_code'"),
            [],
            |row| row.get(0),
        )
        .unwrap();
...
fn pg_type_projection_hides_internal_partition_row_types() {
    ...
    let type_sql =
        rewrite_sql("SELECT typname FROM pg_catalog.pg_type WHERE typname LIKE 'ods_access_log%'")
...
fn pg_index_rewrite_returns_catalog_indexes() {
    ...
    let sql =
        rewrite_sql("SELECT indexrelid, indrelid FROM pg_catalog.pg_index").expect("rewrite sql");
    assert_eq!(route_sql(&sql).unwrap().route, SqlRoute::Read);
...
fn pg_catalog_scalar_functions_rewrite_to_catalog_queries() {
    ...
    .unwrap();
    crate::catalog::execute_catalog_aware_write(&conn, "COMMENT ON TABLE quotes IS 'quotes table'")
        .unwrap();
    crate::catalog::execute_catalog_aware_write(
        &conn,
    ...
    let expr_sql = rewrite_sql("SELECT pg_get_expr('close + 1', 0)").expect("pg_get_expr rewrite");
...
fn pg_namespace_rewrite_returns_duckdb_schemas() {
    let conn = Connection::open_in_memory().unwrap();
    crate::catalog::bootstrap_fresh(&conn).unwrap();
    let sql = rewrite_sql("SELECT oid, nspname FROM pg_catalog.pg_namespace").expect("rewrite sql");
    assert_eq!(route_sql(&sql).unwrap().route, SqlRoute::Read);

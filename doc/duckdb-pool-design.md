...
rsduck 的 managed range partitioned table 需要自动维护物理分区表、null partition、retention 和查询入口。该能力由 partition scheduler 负责发现和调度，但实际修改仍由 single write worker 执行。
...
| manual | admin/operator 手工刷新、修复、清理 null partition。 | SQL/API -> write queue |

timer 触发的维护只负责低频后台巡检，不能成为写入正确性的唯一依赖。写入路径必须能在目标分区缺失时同步创建分区或把不可路由数据写入 null partition。
...
| Job | 作用 |
|-----|------|
| `EnsurePartitionedTable` | 校验 parent 分区表、null partition 和查询入口存在。 |
...
| `CleanupNullPartition` | 显式清理或重放 null partition 中的脏数据。 |

`CleanupNullPartition` 只能由 admin/operator 手工触发，不能由 retention 自动触发。
...
append / insert rows
  ...
  -> invalid/null/unroutable rows -> null partition
...
语义是保留最近 30 个 day partition。scheduler 根据当前时间计算保留窗口，然后只对普通分区执行过期：

```text
active ordinary partitions outside retention window
  -> ExpirePartition
...
- write queue 满时，写入触发的必要分区创建不能静默跳过；写入必须失败或明确写入 null partition。
...
- 不允许 retention 自动删除 null partition。
...
- partition maintenance 指标，包括 pending jobs、last tick、failed jobs、unavailable partitions、null partition row count。

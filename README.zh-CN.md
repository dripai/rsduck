...
rsduck 是一个使用 Rust 封装 DuckDB 的内存数据库中间件服务。它在进程内启动 DuckDB 内存库，对外提供 PostgreSQL wire、MySQL wire 和 Web SQL 控制台，并通过目录快照实现内存库的持久化恢复。

## 功能概览

- 内存 DuckDB：启动后数据主要在内存中读写，适合低延迟分析查询。
- PostgreSQL wire 接入：外部工具可按 PG 协议连接 rsduck。
- MySQL wire 接入：外部工具可按 MySQL 协议连接 rsduck，入口层复用统一认证、权限和 SQL 执行路径。
...
- 高频写入、实时查询的临时分析库。
- 需要 PG 或 MySQL 协议入口，但不想部署完整数据库服务的轻量场景。
...
启用 `mysql.enabled = true` 后，支持 MySQL 协议的工具和驱动可以连接 MySQL wire 端口。MySQL wire、PG wire 和 Web 控制台只是不同入口；认证成功后共用 `DbHandle`、权限校验、typed SQL 执行和结果集元数据路径。
...
当前 MySQL 入口固定声明 `caching_sha2_password`，对应 MySQL 8 默认认证插件；不会根据客户端版本或名称自动降级到 `mysql_native_password`。`CREATE USER` 和 `ALTER USER PASSWORD` 会同时维护 Web / PG 使用的 Argon2 verifier 和 MySQL 使用的 verifier。历史用户如果缺少 MySQL verifier，需要重置密码后再通过 MySQL wire 登录。

已支持 `COM_QUERY`、`COM_PING`、`COM_INIT_DB`、`COM_QUIT`、`COM_STMT_PREPARE`、`COM_STMT_EXECUTE`、`COM_STMT_CLOSE` 和 `COM_STMT_RESET`。Prepared statement 使用 MySQL binary result，标量值从 `SqlValue` 直接编码；`LIST`、`STRUCT`、`MAP`、`ARRAY`、`UNION`、`VARIANT` 等 DuckDB 复杂类型统一按 JSON 暴露，`INTERVAL` 和 `TIMETZ` 按稳定文本类型暴露。

第一版 MySQL wire 不支持 TLS，建议只绑定 `127.0.0.1` 或可信内网地址。
...
- 【mysql.enabled】是否启动 MySQL wire 入口。代码内置默认值为 `false`，仓库示例 `rsduck.toml` 默认启用，便于本地客户端测试。
- 【mysql.bind】MySQL wire 端口监听地址，默认 `127.0.0.1:13306`，避免与本机 MySQL `3306` 冲突。
...
rsduck 使用 Rust 封装 DuckDB，而不是复刻 PostgreSQL 或 MySQL 内核。DuckDB 是唯一 SQL 执行引擎；rsduck 在 DuckDB 外层提供网络入口、PG-compatible 元数据 catalog、认证授权、执行调度、托管 range 分区表和目录快照恢复。
...
src/
  ...
  server/              PG wire、MySQL wire 和 Web server
```
...
```text
client
  -> Web API、PG wire 或 MySQL wire 入口完成认证和协议编解码
  ...
  -> Web API JSON、PG RowDescription/DataRow 或 MySQL resultset 编码
```
...
- Web API、PG wire 和 MySQL wire 都只是入口适配层；认证之后共用同一条 typed SQL 执行与 Describe 路径。
...
- [MySQL Protocol 开发清单](doc/mysql_protocol_todo.md)
...
Windows 服务模式由 WinSW 托管。停止服务时，WinSW 会先向控制台程序发送 Ctrl+C，rsduck 会捕获该信号并在退出前保存一次关闭快照；`rsduck-service.xml` 中的 `stoptimeout` 默认给关闭快照保留 120 秒。
...
关于关闭前快照：rsduck 在 Windows 下处理 Ctrl+C，在 Linux/macOS 等 Unix 系统下同时处理 Ctrl+C/SIGINT 和 SIGTERM。`systemctl stop rsduck`、`launchctl bootout`、控制台 Ctrl+C 都会触发退出前快照。
